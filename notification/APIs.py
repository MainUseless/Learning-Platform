...
def get_unread_notifications():
	...
	stmt = '''SELECT * FROM notifications WHERE stud_id = ?'''
	notifications = c.execute(stmt, (stud_id,)).fetchall()

	unread_notifications = [{'id': n[0], 'message': n[2],'is_read':n[3]} for n in notifications]
...
#     except Exception as e:
#         return Response('error: Internal server error12', status=500)
